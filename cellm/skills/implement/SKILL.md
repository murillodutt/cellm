...
3. **Director Emit** — Before picking a task, check the current phase's `body.specialist.role`. If a Director is registered for the role (e.g., `frontend` → GDU + Engineering Directors, `backend` → Engineering Director, `fullstack` → Engineering Director), call `directive_emit_for_phase` with `{ project, specNodeId: phaseId, projectRoot, objective, specialistRole, pathGlob? }`. This emits mandatory contracts (grep/AST evidence) that the implementation must satisfy. If no Director exists for the role, this is a no-op. After emit, call `directive_list(specNodeId, state='active')` to load the active directives — respect them as hard requirements throughout implementation.
...
- **Skip Director Emit** — when `specialist.role` has a registered Director, always emit directives before implementation. Without directives, violations pass silently through the gate
