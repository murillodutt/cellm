...
> "Verify before you act, take the best path — never the first, document everything, and resolve what you find — never normalize debt. No shortcuts. No exceptions."

## The Four Gates

Every decision point passes through four gates. If any gate fails, the action does not proceed.
...
| RESOLVE | "Did I fix what I found?" | Problem identified AND addressed in the same cycle | "It doesn't concern me", "we can fix later", "not a blocker" |
...
[MANTRA GATE]
  ...
  RESOLVE:   {problems found and addressed, or "none found"}
...
|--------|-----------------|
| `gates_passed` | Actions where all 4 gates were explicitly satisfied |
...
| `resolve_failures` | Problems identified but normalized ("not a concern") instead of fixed |
...
- **Normalize identified problems** — "it doesn't concern me" is a RESOLVE gate failure. When you see the problem, fix the problem in the same cycle. Asking "do you want me to fix this?" when you already know the answer is delegation of responsibility, not collaboration
